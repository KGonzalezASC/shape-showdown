...
// The expansions for each level
const extraEvents: Record<string, string[]> = {
...
for (const [id, lines] of Object.entries(extraEvents)) {
  const idIndex = code.indexOf(`id: '${id}'`);
  if (idIndex === -1) {
    throw new Error(`id not found: ${id}`);
  }
  // Find `const timeline` before idIndex
  const timelineStart = code.lastIndexOf('const timeline', idIndex);
  if (timelineStart === -1) {
    throw new Error(`timeline not found for: ${id}`);
  }
  const timelineEnd = code.indexOf('];', timelineStart);
  if (timelineEnd === -1 || timelineEnd > idIndex) {
    throw new Error(`timeline closing bracket not found for: ${id}`);
  }

  const before = code.slice(0, timelineEnd);
  const after = code.slice(timelineEnd);
  const insertion = lines.join('\n') + '\n  ';
  code = before + insertion + after;
}

fs.writeFileSync(filePath, code, 'utf8');
console.log('Successfully patched all 20 dry timelines in authoredLevels.ts!');

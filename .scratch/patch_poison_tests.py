...
# Ensure imports include BOARD and buildPoisonBeatLevel / applyShopPurchase helpers as needed
if "buildPoisonBeatLevel" not in s:
    s = s.replace(
        "import { applyScriptedShopAttack } from '../shop.js';\n",
        "import { applyScriptedShopAttack, applyShopPurchase } from '../shop.js';\n"
        "import { buildPoisonBeatLevel } from './catalog/authoredLevels.js';\n"
        "import { createEmptyPoisonBoard } from '../puzzleEngine/engine.js';\n"
        "import { makeRng } from '../testHarness/testUtils.js';\n",
    )

# Check if makeRng / createEmptyPoisonBoard / blankGame patterns exist
# Prefer self-contained tests without makeRng if import path wrong.
...
extra = r'''
  ...
  it('scripted wildcard-four applies only after poison is on the stack', () => {
    ...
    // Seed poisonBoard like a locked poisoned piece (multiplayer prerequisite).
...
# Fix imports more carefully - check what exists
# Remove bad imports I may have added
s = p.read_text(encoding="utf8")
...
print("wrote poison synergy tests")

...
const MobileControls: React.FC<MobileControlsProps> = ({ onInput, onAction, onHeightChange }) => {
  ...
  return (
    <div ref={rootRef} className="fixed bottom-0 left-0 right-0 z-50 md:hidden">
      <div className="mx-auto flex w-full max-w-md items-end justify-between gap-3 px-3 pb-[max(0.85rem,env(safe-area-inset-bottom))]">
        ...
        <button
            className="h-14 w-14 rounded-full border-2 border-sky-300/70 bg-sky-500/20 text-[10px] font-extrabold tracking-wide text-sky-100 shadow-[0_0_18px_rgba(14,165,233,0.35)] active:scale-95 active:bg-sky-500/45"
            onTouchStart={tapAction('hardDrop')}
        >
            DROP
        </button>

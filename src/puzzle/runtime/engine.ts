...
import { createInitialPlayerShop, rollShopOnLineClear, tickPlayerShop } from '../../shop/playerShop.js';
import { pushFieldEffect } from '../../shop/fieldEffects.js';
import { MutableRng, RngChannels, ensureRngChannels, makeRng as makeSharedRng, rngNext } from '../../rng.js';
...
function ensureQueue(player: PlayerState, rng: MutableRng): void {
  if (player.finitePieceSourceExhausted) return;
...
export function stepPlayer(
  ...
  if (!player.activePiece) {
    ...
    if (!player.activePiece && options?.finitePieceSource) {
      return result;
    }
